...
const SearchBooks = (filter,config,projection={hash:0}) => {
    return new Promise(async(resolve,reject) => {
        try {
            console.log(config)
            let skip = parseInt(config.skipSteps)
            //Need to put the limit and skip functions
            const results = await Book.find(filter,projection).sort({_id:-1}).count()
            const foundBooks = await Book.find(filter,projection).sort({_id:-1}).limit(4).skip(skip)
            data = {
                results,
                foundBooks
            }
...
const UpdateBook = (bookData, filter) => {
    return new Promise(async(resolve,reject) => {
        try {
            const bookUpdate = await Book.updateOne({barcode: filter}, { $set: {...bookData}})
            resolve (bookUpdate)
        } catch (error) {
            reject(error)
        }
